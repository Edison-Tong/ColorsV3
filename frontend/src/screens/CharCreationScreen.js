...
export default function CharCreationScreen({ route, navigation }) {
  ...
  const [efficientAgainst, setEfficientAgainst] = useState(editChar?.efficient_against || null);
  const [stats, setStats] = useState(
    editChar ? STAT_KEYS.reduce((o, k) => ((o[k] = Number(editChar[k])), o), {}) : baseStats()
  );
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    navigation.setOptions({ title: isEdit ? "Edit Champion" : "Muster a Champion" });
  }, [isEdit]);
  ...
  // Is an Efficiency move among the chosen abilities? If so, the character must pick a target type.
  const hasEfficiency = abilityList.some((a) => a.type === "Efficiency" && abilities.includes(a.name));
  ...
  const pickType = (t) => {
    ...
    setAbilities([]);
    setSpecials([]);
    setEfficientAgainst(null);
  };
  const pickSize = (s) => {
    if ((sizeCounts[s] || 0) >= SIZE_REQ[s])
      return Alert.alert("Limit", `Team already has the max size-${s} characters`);
    setSize(s);
  };
  const pickWeapon = (w) => {
    setWeapon(w);
  ...
  const toggleAbility = (n) =>
    setAbilities((p) => (p.includes(n) ? p.filter((x) => x !== n) : p.length >= 2 ? p : [...p, n]));
  const toggleSpecial = (n) =>
    setSpecials((p) => (p.includes(n) ? p.filter((x) => x !== n) : p.length >= 3 ? p : [...p, n]));
  ...
  const validate = () => {
    ...
    if (hasEfficiency && !efficientAgainst) return "Choose what your Efficiency move is strong against";
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      base_weapon: weapon,
      abilities,
      specials: type === "mage" ? specials : [],
      stats,
      efficient_against: hasEfficiency ? efficientAgainst : null,
    };
    try {
      ...
      <Field label="Type">
        <View style={styles.chipRow}>
          ...
          <Chip
            active={type === "mage"}
            disabled={mageFull}
            onPress={() => pickType("mage")}
            label={`✨ Mage${mageFull ? " (full)" : ""}`}
          />
        </View>
      </Field>
        ...
        <Field label="Weapon">
          <View style={styles.chipRow}>
            {weaponList.map((w) => (
              <Chip
                key={w.value}
                active={weapon === w.value}
                onPress={() => pickWeapon(w.value)}
                label={`${WEAPON_GLYPH[w.value]} ${w.label}`}
              />
            ))}
          </View>
        </Field>
        ...
        <Field label={`Weapon Abilities (${abilities.length}/2)`}>
          {abilityList.map((a) => (
            <AbilityRow
              key={a.name}
              a={a}
              active={abilities.includes(a.name)}
              onPress={() => toggleAbility(a.name)}
              efficientAgainst={efficientAgainst}
              onPickEfficiency={setEfficientAgainst}
            />
          ))}
        </Field>
      )}
        ...
        <Field label={`Mage Special Abilities (${specials.length}/3)`}>
          {specialList.map((a) => (
            <AbilityRow
              ...
              a={a}
              special
              active={specials.includes(a.name)}
              onPress={() => toggleSpecial(a.name)}
            />
          ))}
        </Field>
      )}
      ...
      <Field label="Stats">
        <View style={[styles.totalBar, total === STAT_CAP && { borderColor: theme.good }]}>
          <Text style={styles.totalText}>
            Points used:{" "}
            <Text style={{ color: total === STAT_CAP ? theme.good : theme.warn, fontWeight: "800" }}>
              {total}/{STAT_CAP}
            </Text>
          </Text>
        </View>
        {STAT_KEYS.map((k) => (
          <View key={k} style={styles.statRow}>
            ...
            <View style={styles.stepper}>
              ...
              <Stepper
                symbol="+"
                disabled={(k !== "health" && stats[k] >= STAT_MAX) || total >= STAT_CAP}
                onPress={() => changeStat(k, 1)}
              />
            </View>
          </View>
        ))}
      ...
      {preview && (
        <Torn style={styles.previewCard}>
          ...
          <Text style={styles.previewText}>
            Power {preview.power} · Prot {type === "mage" ? preview.protection.magic : preview.protection.melee} · Acc{" "}
            {preview.accuracy} · Eva {preview.evasion} · Crit {preview.critical} · Block {preview.block}
          </Text>
        </Torn>
      )}
...
const AbilityRow = ({ a, active, special, onPress, efficientAgainst, onPickEfficiency }) => {
  const [open, setOpen] = useState(false);
  const isEff = a.type === "Efficiency";
  const curLabel = efficientAgainst ? weaponsData.weapons[efficientAgainst]?.label : null;
  return (
    <TouchableOpacity onPress={onPress} style={[styles.abRow, active && styles.abRowActive]}>
      <View style={{ flex: 1 }}>
        <Text style={styles.abName}>
          {a.name} {active ? "✓" : ""}
        </Text>
        <Text style={styles.abEffect}>{special ? a.description : a.effect}</Text>
        <Text style={styles.abMeta}>
          {special ? `range ${a.range} · ${a.effect}` : `hit ${a["hit%"]}% · range ${a.range} · ${a.type}`}
        </Text>

        {isEff && active && (
          <TouchableOpacity style={styles.effPill} onPress={() => setOpen(true)}>
            <Text style={styles.effPillText}>{curLabel || "choose…"} ▾</Text>
          </TouchableOpacity>
        )}

        {isEff && (
          <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
            <TouchableOpacity style={styles.effBackdrop} activeOpacity={1} onPress={() => setOpen(false)}>
              <View style={styles.effPicker}>
                <Text style={styles.effPickerTitle}>Efficient against…</Text>
                <ScrollView style={{ maxHeight: 320 }}>
                  {Object.values(weaponsData.weapons).map((w) => (
                    <TouchableOpacity
                      key={w.value}
                      style={[styles.effOpt, efficientAgainst === w.value && styles.effOptActive]}
                      onPress={() => {
                        onPickEfficiency(w.value);
                        setOpen(false);
                      }}
                    >
                      <Text style={styles.effOptText}>
                        {WEAPON_GLYPH[w.value] || "⚔️"} {w.label}
                        {w.type === "magick" ? " (mage)" : ""}
                      </Text>
                    </TouchableOpacity>
                  ))}
...
const styles = StyleSheet.create({
  ...
  effPillText: { color: theme.good, fontWeight: "700", fontSize: 12.5 },
  effBackdrop: { flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", alignItems: "center" },
  effPicker: {
    width: 280,
  ...
  effPickerTitle: { fontFamily: FONTS.heading, color: theme.gold, fontSize: 15, marginBottom: 8, letterSpacing: 0.5 },
  effOpt: { paddingVertical: 9, paddingHorizontal: 10, borderRadius: 8, marginBottom: 4 },
  effOptActive: { backgroundColor: theme.primary },
  effOptText: { color: theme.text, fontSize: 14, fontWeight: "600" },
